...
Static group authorization should perform as expected.',
    ...
    if (modelDirective) {
      ...
      this.convertRulesToRoles(acm, staticRules, false, typeFieldName, ['list', 'get']);
  ...
  transformSchema = (context: TransformerTransformSchemaStepContextProvider): void => {
    ...
    const getOwnerFields = (acm: AccessControlMatrix) => acm.getRoles().reduce((prev: string[], role: string) => {
      if (this.roleMap.get(role)!.strategy === 'owner') prev.push(this.roleMap.get(role)!.entity!);
      return prev;
    }, []);
    for (const [modelName, acm] of this.authModelConfig) {
    ...
    for (const [typeFieldName, acm] of this.authNonModelConfig) {
    ...
    if (searchableAggregateServiceDirectives.size > 0) {
      ...
      for (const aggType of SEARCHABLE_AGGREGATE_TYPES) {
  ...
  generateResolvers = (context: TransformerContextProvider): void => {
    ...
    for (const [modelName, acm] of this.authModelConfig) {
      ...
      for (const query of queryFields.values()) {
        switch (query.type) {
      ...
      if (context.metadata.has(indexKeyName)) {
        for (const index of context.metadata.get<Set<string>>(indexKeyName)!.values()) {
      ...
      const readRoles = [...new Set([...acm.getRolesPerOperation('get'), ...acm.getRolesPerOperation('list')])];
      ...
      for (const field of modelFields) {
        ...
        const allowedRoles = fieldReadRoles.filter(
          r => acm.isAllowed(r, field.name.value, 'get') || acm.isAllowed(r, field.name.value, 'list'),
        );
      ...
      for (const mutation of mutationFields.values()) {
      ...
      for (const subscription of subscriptionFieldNames) {
  ...
  protectSchemaOperations = (
    ...
    addServiceDirective(ctx.output.getQueryTypeName()!, 'list', modelConfig?.queries?.sync);
    ...
    if (ctx.metadata.has(indexKeyName)) {
      for (const index of ctx.metadata.get<Set<string>>(indexKeyName)!.values()) {
    ...
    if (searchableDirective) {
      ...
      addServiceDirective(ctx.output.getQueryTypeName(), 'list', config.queries.search);
    }
    ...
    if (subscriptions?.level === SubscriptionLevel.on) {
      ...
      if (subscriptions.onCreate && modelConfig?.mutations?.create) {
        for (const onCreateSub of subscriptions.onCreate) {
          addServiceDirective(ctx.output.getSubscriptionTypeName()!, 'list', onCreateSub);
          addSubscriptionArguments(ctx, onCreateSub, subscriptionArguments);
        }
      }
      if (subscriptions.onUpdate && modelConfig?.mutations?.update) {
        for (const onUpdateSub of subscriptions.onUpdate) {
          addServiceDirective(ctx.output.getSubscriptionTypeName()!, 'list', onUpdateSub);
          addSubscriptionArguments(ctx, onUpdateSub, subscriptionArguments);
        }
      }
      if (subscriptions.onDelete && modelConfig?.mutations?.delete) {
        for (const onDeleteSub of subscriptions.onDelete) {
          addServiceDirective(ctx.output.getSubscriptionTypeName()!, 'list', onDeleteSub);
  ...
  protectRelationalResolver = (
    ...
    if (this.authModelConfig.has(relatedModelObject.name.value)) {
      ...
      const roleDefinitions = [
        ...new Set([
          ...acm.getRolesPerOperation('list').map(r => this.roleMap.get(r)!),
          ...acm.getRolesPerOperation('get').map(r => this.roleMap.get(r)!),
        ]),
      ];
  ...
  */
  protectSearchResolver = (
    ...
    const readRoleDefinitions = acm.getRolesPerOperation('list').map(role => {
      const allowedFields = acmFields.filter(resource => acm.isAllowed(role, resource, 'list'));
  ...
  */
  ...
  private doesTypeHaveRulesForOperation(acm: AccessControlMatrix, operation: ModelOperation) {
    const rolesHasDefaultProvider = (roles: Array<string>) => roles.some(r => this.roleMap.get(r)!.provider! === this.configuredAuthProviders.default);
  ...
  */
  ...
  private addFieldsToObject(ctx: TransformerTransformSchemaStepContextProvider, modelName: string, ownerFields: Array<string>) {
    ...
    for (const ownerField of ownerFieldsToAdd) {
  ...
  private getServiceDirectives(providers: Readonly<Array<AuthProvider>>, addDefaultIfNeeded = true): Array<DirectiveNode> {
    ...
    */
    ...
    for (const [authProvider, directiveName] of AUTH_PROVIDER_DIRECTIVE_MAP) {
