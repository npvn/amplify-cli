...
test('support for belongs to linked to hasMany with primary key and sort key fields', () => {
  ...
  const sortKeyField = blogType.fields.find((f: $TSAny) => f.name.value === 'wea');
  ...
  expect(createBlogInput.fields.find((f: $TSAny) => f.name.value === 'wea')).toBeDefined();
  ...
  expect(updateBlogInput.fields.find((f: $TSAny) => f.name.value === 'wea')).toBeDefined();
});
